...
void writeCompressedOutput(const char* inFile,
  ...
  int cha;
  while ((cha = input.get()) != EOF) {
    std::map<unsigned,BitString>::const_iterator it = theMap.find(cha);
    if (it != theMap.end()) {
      bfw.writeBitString(it->second);
    }
    else {
  ...
  std::map<unsigned,BitString>::const_iterator got = theMap.find(256);
  bfw.writeBitString(got->second);
  input.close();
}
